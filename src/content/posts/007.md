...
# 10. 프로젝트에서 바로 확인할 체크리스트

실제 프로젝트에서는 다음 항목을 주기적으로 확인하는 것이 좋다.

- 사용 중인 프레임워크와 라이브러리 버전을 확인했는가?
- 알려진 취약점이 있는 패키지를 방치하고 있지 않은가?
- 사용자 입력값을 서버에서 다시 검증하는가?
- SQL을 문자열 결합으로 만들지 않는가?
- HTML 삽입 기능을 사용할 때 정제 과정을 거치는가?
- 인증과 인가를 구분해서 처리하는가?
- 비밀번호를 평문으로 저장하지 않는가?
- 운영 환경에서 내부 에러 정보가 노출되지 않는가?
- 보안 패치 후 기능 테스트를 진행하는가?

체크리스트는 특별한 보안 도구가 없어도 적용할 수 있다. 중요한 것은 기능 구현이 끝난 뒤 보안을 따로 보는 것이 아니라, 개발 과정 안에 보안 확인을 포함하는 것이다.
